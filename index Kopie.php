...
$app->get('/test/gd', function () use ($app, $config) {

});

$app->get('/test/imagick', function () use ($app, $config) {

});

$app->get('/p/:size', function ($size) use ($app, $config) {
    ...
    $images = Helper::getRawImages();

    $rand_img = array_rand($images);

    $img_info = getimagesize($images[$rand_img]);
    $orginal_size['width']  = $img_info[0];
    $orginal_size['height'] = $img_info[1];

    $new_size = max($size_y, $size_x);

    // $size = Helper::getImageScale($orginal_size['width'], $orginal_size['height'], $new_size);

    $im = new Imagick();
    $im->readImageBlob(file_get_contents($images[$rand_img]));
    $im->setImageFormat('jpg');
    $im->setImageCompressionQuality(100);
    $im->resizeImage($size_x, $size_y, NULL, 1);
